...
def find_similar_images_euclidean(embeddings_db, query_embedding, top_k=5):
    """
    Finds the top K most similar images to a given query embedding using Euclidean distance.
    ...
    Returns:
        list: A list of tuples (image_name, distance) sorted by distance in ascending order.
    """
    similarity_scores = {}
    for image_name, data in embeddings_db.items():
        embedding = data["embedding"]
        dist = euclidean_distances(query_embedding, embedding)[0][0]
...
similar_images = find_similar_images_euclidean(embeddings_db, query_embedding, top_k=5)
